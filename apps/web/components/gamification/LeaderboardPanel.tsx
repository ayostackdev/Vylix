              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 rounded-xl px-3 py-2.5 transition-all ${
                    isMe ? 'bg-emerald-50 ring-1 ring-emerald-200' : 'hover:bg-gray-50'
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-xs font-semibold truncate ${isMe ? 'text-emerald-700' : 'text-gray-900'}`}>
                      {entry.full_name} {isMe && <span className="text-[10px] font-bold text-emerald-600">(you)</span>}
                    </p>
                  </div>
                  <span className={`text-xs font-bold shrink-0 ${tab === 'streaks' ? 'text-orange-600' : 'text-emerald-600'}`}>
